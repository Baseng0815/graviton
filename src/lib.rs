...
struct State<'a> {
    ...
    num_pentagon_vertices: u32,
    num_pentagon_indices: u32,
    triangle_vertex_buffer: wgpu::Buffer,
    num_triangle_vertices: u32,
    draw_triangle: bool,
}
...
impl<'a> State<'a> {
    ...
    async fn new(window: &'a Window) -> State<'a> {
        ...
        let pentagon_vertex_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Vertex Buffer"),
            contents: bytemuck::cast_slice(PENTAGON_VERTICES),
            usage: wgpu::BufferUsages::VERTEX,
        });

        let pentagon_index_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Index Buffer"),
            contents: bytemuck::cast_slice(PENTAGON_INDICES),
            usage: wgpu::BufferUsages::INDEX,
        });

        let num_pentagon_vertices = PENTAGON_VERTICES.len() as u32;
        let num_pentagon_indices = PENTAGON_INDICES.len() as u32;
        ...
        let num_triangle_vertices = TRIANGLE_VERTICES.len() as u32;
        ...
        Self {
            ...
            num_pentagon_vertices,
            num_pentagon_indices,
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        if self.draw_triangle {
            ...
            render_pass.draw(0..self.num_triangle_vertices, 0..1);
        } else {
            render_pass.set_vertex_buffer(0, self.pentagon_vertex_buffer.slice(..));
            render_pass.set_index_buffer(
                self.pentagon_index_buffer.slice(..),
                wgpu::IndexFormat::Uint16,
            );
            render_pass.draw_indexed(0..self.num_pentagon_indices, 0, 0..1);
        }
                        ...
                        WindowEvent::RedrawRequested => {
                            ...
                            match state.render() {
                                ...
                                Err(wgpu::SurfaceError::Lost | wgpu::SurfaceError::Outdated) => {
                                    log::info!("Surface resize to {:?}", state.size);
                            ...
                            if event.state.is_pressed() {
                                match event.physical_key {
                                    PhysicalKey::Code(KeyCode::Space) => {
                                        // toggle between pentagon and triangle
